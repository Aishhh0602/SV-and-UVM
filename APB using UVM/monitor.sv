class apb_monitor extends uvm_monitor;
  ...
  task run_phase(uvm_phase phase);
    forever
      begin
        @(apbif.mon_cb)
   		 req.psel =apbif.mon_cb.psel;
       	 req.penable = apbif.mon_cb.penable;
         req.pwrite = apbif.mon_cb.pwrite;
    	 req.paddr = apbif.mon_cb.paddr;
         //req.pready = vif.mon_cb.pready;
      end
    
    if(apbif.mon_cb.pwrite)
      begin
        req.pwdata = apbif.mon_cb.pwdata;
        `uvm_info(get_type_name(), $sformatf("Captured WRITE to addr=0x%0h data=0x/%0h",req.paddr,req.pwdata), UVM_MEDIUM)
    end
    else
      begin
        req.prdata = apbif.mon_cb.prdata;
